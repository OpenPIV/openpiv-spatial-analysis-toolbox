...
%TIMEBOX Time Analysis Toolbox, part of the 
...
%                                 handles.quantity = 1/i*(handles.quantity*(i-1) + corrcoef(tmp));     % averaging
    ...
    case 6 % spectrum in t
        switch handles.method
            case {1,2} % fft(corr) or PSD, not implemented yet
                [tmp,lags] = crosscorr(squeeze(handles.property(handles.data.i(1),handles.data.j(1),:)));    % time direction
                for i = 2:length(handles.data.i)
                ...
                handles.spectrum_xaxis = (0:ceil((handles.nfft+1)/2-1))*handles.Fs/handles.nfft;
...
%                 end
            case 3 % freqz
                ...
                y = squeeze( handles.property(handles.data.i(1),handles.data.j(1),:) );
                freqRange = linspace(0,handles.Fs/2,handles.nfft);
                tmp = freqz(y,1,freqRange,handles.Fs)/length(y);    % time direction
                for i = 2:length(handles.data.i)
                    y = squeeze(handles.property(handles.data.i(i),handles.data.j(i),:));
                    tmp = 1/i*(tmp*(i-1) + freqz(y,1,freqRange,handles.Fs)/length(y));
                end
                % i = find(lags >= 0);
                % tmp = 2*real(tmp(i))/length(tmp);
                w = windowvector(length(tmp),get(handles.window,'Value'));
                tmp = tmp(:).*w(:);
                handles.quantity = abs(tmp);
                handles.spectrum_xaxis = freqRange; % 2*pi*(0:ceil((handles.nfft+1)/2-1))*handles.Fs/handles.nfft;
                % handles.quantity = handles.quantity(1:ceil((handles.nfft+1)/2));
                % handles.quantity = handles.quantity(2:end);
                % handles.spectrum_xaxis = handles.spectrum_xaxis(2:end);
        ...
        handles.quantity = handles.quantity(2:end);
        handles.spectrum_xaxis = handles.spectrum_xaxis(2:end);
        handles.spectrum_xlabel = 'Frequency [Hz]';
    ...
    case 7 % spectrum in k1
        switch handles.method
            case 1 % fft(corr)
                ...
                tmp = tmp(:).*w(:);
                handles.quantity = (1/pi/handles.Fs)*real(fft(tmp,handles.nfft))./(w'*w);
                handles.spectrum_xaxis = (0:ceil((handles.nfft+1)/2-1))/(abs(handles.data.dx))/handles.nfft;
                handles.quantity = handles.quantity(1:ceil((handles.nfft+1)/2));
                handles.quantity = handles.quantity(2:end);
                handles.spectrum_xaxis = handles.spectrum_xaxis(2:end);
    ...
    case 8 % spectrum in k2
        switch handles.method
            case 1 % fft(corr)
                ...
                tmp = tmp(:).*w(:);
                handles.quantity = (1/pi/handles.Fs)*real(fft(tmp,handles.nfft))./(w'*w);
                handles.spectrum_xaxis = (0:ceil((handles.nfft+1)/2-1))/(abs(handles.data.dy))/handles.nfft;
                handles.quantity = handles.quantity(1:ceil((handles.nfft+1)/2));
                handles.quantity = handles.quantity(2:end);
                handles.spectrum_xaxis = handles.spectrum_xaxis(2:end);
...
switch get(handles.popupmenu_quantity,'Value')
    case 6 % none
        set(handles.popupmenu_method,'Enable','On');
    otherwise
        set(handles.popupmenu_method,'Value',1);
        set(handles.popupmenu_method,'Enable','inactive');
end
