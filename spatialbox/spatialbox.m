    ...
    if get(handles.checkbox_fluct,'Value') == 1
        ...
        switch get(handles.popupmenu_quantity,'Value')
            ...
            case 2 % uf^2
                ...
                %                                handles.units='[m/s]^2';
                handles.units = [handles.velUnits,'^2']; % '[m/s]^2';
            case 3 % vf^2
...
try
    curdir = cd;
    [coordMatfile,coordMatpath] = uigetfile('*.mat','Coordinates file');
    load(fullfile(coordMatpath,coordMatfile));


    %     loadVec('F:\Markus\PIV_new\coordinates_61_end.mat')
    if exist('x','var')
        handles.x = x;
        handles.y = max(y(:)) - y;
        clear x y
    ...
    [coordMatfile,coordMatpath] = uigetfile(fullfile(coordMatpath,'*.mat'),'Velocity file');
    load(fullfile(coordMatpath,coordMatfile));
    % loadVec('F:\Markus\PIV_new\vel_field_61_end.mat')
    if exist('fu','var')
        handles.u = fu;
        handles.v = fv;
        clear fu fv
...
try
    ...
    if ~isempty(findstr(gui_files,'_noflt'))
        data = '_noflt.txt';
    elseif ~isempty(findstr(gui_files,'_flt'))
        data = '_flt.txt';
    else
        data = 'txt';
    end

    gui_files = ReadTXTDir(gui_path,data);
    ...
    %%%%%%%%%%%%%%%%
    d = load(fullfile(gui_path,gui_files{1}));
    d = repmat(d,[1 1 handles.N]);
    for i = 2:handles.N
        d(:,:,i) = load(fullfile(gui_path,gui_files{i}));
    end
...
if w.bytes > 60000000
    ...
    %
    %
    % rmfield(handles,{'uf','vf','dudx','dudy','dvdx','dvdy'});
    %
    % save('tmp.mat','handles');
    % clear all
    % load('tmp.mat');
    %
    %
    % handles.x = handles.x(yind,handles.xind);
    % handles.y = handles.y(yind,handles.xind);
    % handles.u = handles.u(yind,handles.xind,:);
    % handles.v = handles.v(yind,handles.xind,:);
    %
    % [rows,cols,handles.N] = size(handles.u);
    %
    % % handles.dudx = handles.dudx(yind,handles.xind,:);
    % % handles.dudy = handles.dudy(yind,handles.xind,:);
    % % handles.dvdx = handles.dvdx(yind,handles.xind,:);
    % % handles.dvdy = handles.dvdy(yind,handles.xind,:);
    % %
    % % handles.uf = handles.uf(yind,handles.xind,:);
    % % handles.vf = handles.vf(yind,handles.xind,:);
    %
    % rmfield(handles,{'uf','vf','dudx','dudy','dvdx','dvdy'});
    %
    % handles.uf = zeros(rows,cols,handles.N);
    ...
    %
    % % Derivatives
    % [handles.dudx,handles.dudy,handles.dvdx,handles.dvdy] = ...
    %     deal(zeros(rows,cols,handles.N+1));
    % % handles.dudy = zeros(rows,cols,handles.N+1);
    % % handles.dvdx = zeros(rows,cols,handles.N+1);
    % % handles.dvdy = zeros(rows,cols,handles.N+1);
    % %
